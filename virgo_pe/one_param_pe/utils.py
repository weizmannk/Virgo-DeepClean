...
def get_gwpy_mean_psd(
    psd_data, fs=4096, fftlength=32, low_frequency=16, method="welch"
):
...
def pe_on_grid(
    ...
    injection_parameters["geocent_time"] = (
        injection_parameters["geocent_time"] + geocent_time_correction
    )
    ...
    mc_inj = bilby.gw.conversion.component_masses_to_chirp_mass(
        injection_parameters["mass_1"], injection_parameters["mass_2"]
    )
    z = bilby.gw.conversion.luminosity_distance_to_redshift(
        injection_parameters["luminosity_distance"]
    )
    mass_ratio_inj = bilby.gw.conversion.component_masses_to_mass_ratio(
        injection_parameters["mass_1"], injection_parameters["mass_2"]
    )
    ...
    if analysis_data_channel == None:
        analysis_data = TimeSeries.read(
            analysis_data_frame, start=start_time, end=end_time
        )
    else:
        analysis_data = TimeSeries.read(
            analysis_data_frame, analysis_data_channel, start=start_time, end=end_time
        )
        ...
        if psd_data_frame is not None:
            if psd_data_channel == None:
                psd_data = TimeSeries.read(
                    psd_data_frame, start=psd_start_time, end=psd_end_time
                )
            else:
                psd_data = TimeSeries.read(
                    psd_data_frame,
            ...
            np.savetxt(
                fname=psd_file, X=np.array([psd_gwpy[0][:-1], psd_gwpy[1][:-1]]).T
            )
    ...
    if theta == "geocent_time":
        ...
        priors[theta] = bilby.core.prior.Uniform(
            name=theta, minimum=theta_min, maximum=theta_max, boundary="reflective"
        )
